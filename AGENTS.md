...
- **Runtime profile override** → `runtime-profile.json` is the SSoT. `oc profile pentest` keeps every agent/category available, uses Flash 0731 for economical work, Pro 0813 for explicit depth roles, and GLM 5.3 only as fallback except `ultrabrain` (GLM primary, Pro fallback). `oc profile normal` restores the broader matrix above. Profile state and generated configs live under `~/.local/state/openconfig`; switching must never rewrite tracked source files.
